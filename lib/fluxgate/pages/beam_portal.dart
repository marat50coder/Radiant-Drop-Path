...
/// Full-screen WebView shell (gray mode). Applies every native-feel + safe
/// area + cold-start fix from the lessons catalogue.
class BeamPortal extends StatefulWidget {
  const BeamPortal({
    super.key,
    required this.url,
    required this.vault,
  ...
  @override
  State<BeamPortal> createState() => _BeamPortalState();
}

class _BeamPortalState extends State<BeamPortal> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    widget.beacon.onDestination = (url) {
  ...
  Future<void> _settleColdViewport() async {
    ...
    // nudge here — that made a cold-start push link open sideways then flip.
    // Any residual stretch is fixed after load by the resize + single reload.
  ...
  NavigationDelegate _navigation() {
    return NavigationDelegate(
      ...
      onWebResourceError: (error) {
        ...
        // never silently swallowed (that left the app "frozen").
  ...
  /// Routes to the offline screen immediately (no probe). Guarded by
  /// [_offlineShown] so multiple triggers are safe.
